...
double N(double x){
    double gamma = 0.2316419, a2 =-0.356563782, a1 = 0.319381530;
    double a4 =-1.821255978, a3 = 1.781477937, a5 = 1.330274429;
    double pi = 4.0*atan(1.0), k = 1.0/(1.0+gamma*x);
    
    if (x>=0.0) return 1.0-((((a5*k+a4)*k+a3)*k+a2)*k+a1)*k*exp(-x*x/2.0)/sqrt(2.0*pi);
    return 1.0-N(-x);
}
